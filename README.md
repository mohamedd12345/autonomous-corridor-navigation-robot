...
The robot reads distances from three sensors and converts those readings into a small set of discrete states. The FSM uses those states to decide whether to drive forward, slow down, reverse, or steer left or right. The code is compact and designed to be easy to tune.
...
| MPU6050              | IMU used for yaw measurements (optional for the basic FSM)   |
| HC-SR04 (x2)         | Left and right ultrasonic sensors                            |
| Steering servo       | Controls steering angle                                      |
| DC motors            | One for forward drive and one for reverse/brake (PWM control)|
...
Each loop the code reads the three sensors and converts the numeric distances to three binary variables:
...
- Front ToF: readings are smoothed with a simple exponential moving average (`filtered = 0.7*previous + 0.3*new`). You can change the smoothing weights for faster or more stable response.
- Thresholds (300 mm for side sensors, 250 mm for front) are starting points. If your robot is larger or faster, reduce thresholds or add hysteresis to avoid oscillation.

Future improvements

- Use IMU yaw to correct and maintain heading using a PID controller.
- Add rate-limited acceleration and deceleration for smoother motion.
- Log sensor data over serial for debugging and analysis.
